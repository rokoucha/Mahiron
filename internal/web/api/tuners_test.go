...
func TestGetTunersAndGetTuner(t *testing.T) {
	handler := NewHandler(HandlerConfig{TunerManager: tuner.NewTunerManager(&tuner.ManagerConfig{
...
func TestGetTunersIncludesRemoteTuners(t *testing.T) {
	localTuners := tuner.NewTunerManager(&tuner.ManagerConfig{TunersConfig: config.TunersConfig{
		{Name: "local-first", Types: []string{"BS"}, Command: "sleep 1"},
	}})
	streamManager := remoteTunerStatusProvider{Manager: stream.NewStreamManager(stream.ManagerConfig{TunerManager: localTuners})}
...
func TestChannelStreamReturnsTrackedTunerUserID(t *testing.T) {
	...
	tunerManager := tuner.NewTunerManager(&tuner.ManagerConfig{TunersConfig: config.TunersConfig{
	...
	handler := NewHandler(HandlerConfig{
		...
		StreamManager: stream.NewStreamManager(stream.ManagerConfig{
