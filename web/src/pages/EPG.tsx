...
export default function EPG({ dashboard }: { dashboard: DashboardState }) {
  ...
  const programsByService = useMemo(() => {
    ...
    for (const program of programs.data ?? []) {
      if (
        program.startAt + program.duration < windowStart ||
  ...
  const epgColumns = useMemo(
    () => makeEpgColumns(visibleServices, programsByService),
    [visibleServices, programsByService],
  )
          ...
          <div className="timeline-body" style={{ height: timelineHeight }}>
            <div className="time-rail">
              {Array.from({ length: 7 }).map((_, index) => {
                ...
                const showsDate =
                  index > 0 && !isSameDate(time, time - 60 * 60 * 1000)
                return (
                  <span
                    className={showsDate ? 'day-boundary' : ''}
                    key={time}
            ...
            >
              <div
                className="now-line"
                style={{ top: ((now - windowStart) / 60000) * pxPerMinute }}
              />
              {epgColumns.map((column, index) => {
                const columnPrograms = makeEpgProgramBlocks(
                  column,
                  programsByService,
                )
                return (
                  <div
                    className="program-lane"
                    key={column.key}
                    style={{ left: index * serviceWidth, width: serviceWidth }}
                  >
                    {columnPrograms.map(({ key, program }) => {
                      ...
                      const top =
                        ((visibleStart - windowStart) / 60000) * pxPerMinute
                      const height = Math.max(
                        8,
                        ((visibleEnd - visibleStart) / 60000) * pxPerMinute,
                      )
        ...
        {selected && (
          <ProgramModal
            ...
            service={serviceMap.get(
              `${selected.networkId}:${selected.serviceId}`,
            )}
